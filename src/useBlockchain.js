...
const rest = pcl.restClient.createRestClient( process.env.REACT_APP_NODE_URL, process.env.REACT_APP_BLOCKCHAIN_RID, 5)
const gtx = pcl.gtxClient.createClient(rest,Buffer.from(
...
export function registerUser(user,username){
    const pubKey = Buffer.from(user.pubKey);
    const privKey = Buffer.from(user.privKey);
    const rq = gtx.newTransaction([pubKey]);
    rq.addOperation("register_user", pubKey,username);
    rq.sign(privKey, pubKey);
    return rq.postAndWaitConfirmation();
...
export async function createTestItem(user){
    ...
    const pubKey = Buffer.from(user.pubKey);
    const privKey = Buffer.from(user.privKey);
    const total_supply=await getAllItems();
...
export function sellItem(user,price,id){
    const pubKey = Buffer.from(user.pubKey);
    const privKey = Buffer.from(user.privKey);
    const rq = gtx.newTransaction([pubKey]);
